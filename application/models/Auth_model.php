...
class Auth_model extends CI_Model {
	...
	public function reigster($email, $password,$fname,$lname)
	{
		$passStr = $password;
		...
		$linkPath  = base_url('verifyEmail/').base64_encode($email);
		$subject   = 'Registration Successful';
		$template  = $this->load->view('emailtemplates/register','',true);

		$tempStr   = ['{{linkPath}}','{{tempPassword}}'];
		$tempRplc  = [$linkPath,$passStr];

		$template  = str_replace($tempStr,$tempRplc,$template);

		// $template = str_replace(array('{{linkPath}}', $linkPath), array('{{tempPassword}}', $password), $template);
	...
	public function forgetpassword($email = null)
	{
		if($this->checkuser($email)){
			$temp = uniqid();
			$temp_data = base64_encode($temp);
			$this->db->set("fp_token", $temp)->where("email", $email)->update("users");
			$this->mail($email, "<h1>Welcome to PnaPna.<h1><br><h3> Your password reset link is below please click on link and reset your password</h3><br><p> Click here: <a href='".base_url('reset-password/'.$temp_data)."'>Reset Password</a><p>", "Forgot Password");
			return $temp_data;
		}else{
			return 0;
		}
	...
	public function resetpassword($fp_token = null, $password = null)
	{
		if($password != null && $fp_token != null){
			$password = app_hasher()->HashPassword($password);
			$temp_data = $this->db->where("fp_token", $fp_token)->get("users")->num_rows();
			if($temp_data == 1){
				$this->db->set("password", $password)->set("fp_token", '')->where("fp_token", $fp_token)->update("users");
			}
			// $this->mail($email, "<h1>Welcome to PnaPna.<h1><br><h3> Your password has beed rest successfully.</h3><br><p> Click here: <a href='".base_url('forget-password/'.$temp_data)."'>Reset Password</a><p>", "Reset Password");
			return 1;
		}else{
			return 0;
		}
